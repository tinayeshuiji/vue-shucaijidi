<template>
  <div id="home-box" v-cloak>
    ...
    <div v-if="shopList.length==0" class="aui-flex-row aui-flex-middle aui-flex-center nothing-box">
...
export default {
  ...
  methods: {
    ...
    getHotList: function() {
      ...
      api.ajax(
        {
          url: vuedata.host + "/api/api/getWordList",
          method: "post",
    ...
    getKeySearch: function(word) {
      ...
      // alert(word)
      if (word !== "") {
        vuedata.searchContent = word;
        if (vuedata.historyList.length >= 10) {
          vuedata.historyList.splice(9, 1);
        }
        for (var i = 0; i < vuedata.historyList.length; i++) {
          if (vuedata.historyList[i] == vuedata.searchContent) {
            vuedata.historyList.splice(i, 1);
          }
        }
        vuedata.historyList.unshift(vuedata.searchContent);
        $api.setStorage("historyList", vuedata.historyList);
      } else {
      ...
      //         });
      //         vuedata.shopList = [];
      //           vm.openSeachFrame(vuedata.shopList)
      //     }
      //
      // });
    },
    ...
    goSearch: function() {
      ...
      vuedata.bp = 3;
      vuedata.discount = 3;
      vuedata.filterShow = true;
      api.closeFrame({
        name: "search_frame"
      });
    },
    openSeachFrame: function(searchContent) {
      api.openFrame({
        name: "search_frame",
        ...
        rect: {
          ...
          y: vuedata.headH,
          w: "auto",
          h: vuedata.height - vuedata.headH
        },
        pageParam: {
    ...
    getAjax: function() {
      // alert("price:"+vuedata.bp+"+discount:"+vuedata.discount+"+salf:"+vuedata.filter+"condition："+vuedata.condition+"word:"+vuedata.searchContent+"inStock:"+vuedata.inStock)
      api.ajax(
        {
          url: vuedata.host + "/api/goods/getSearchGoodsList",
          method: "post",
